...
function OsuNoteSkin:load()
	...
	if not SplitStages then
		self:addStages(columns[1], columns[ninputs] + width[ninputs] + space[ninputs + 1], self.fullWidth)
	else
		self:addStages(columns[1], columns[ninputs2] + width[ninputs2], widthLeft)
		self:addStages(
...
function OsuNoteSkin:addAccuracy()
	...
	local accObj = self.playField:addAccuracy(ImageValueView({
		transform = self.playField:newTransform(1024, 768, "right"),
		x = 1016,
		y = 0,
		scale = 0.6,
